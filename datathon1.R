...
RI_New = RI %>% 
  ...
  select("RegionID","yr/mon", "RI")



VI_RI = RI_New %>% 
  left_join(VI_New, by = c("RegionID"="RegionID", "yr/mon"="yr/mon"))
...
FinalData_Left = VI_RI_Mor %>% 
  left_join(Zip, by = c("RegionID"="Zip_Code", "Year"="Year"))

FinalData_Inner = VI_RI_Mor %>% 
  inner_join(Zip, by = c("RegionID"="Zip_Code", "Year"="Year"))
...
write.csv(FinalData_Inner, "DatathonData.csv")
...
write.csv(afford_rent1, "affordability.csv")
